...
```bash
npm test            # Vitest, ~11 с
npm run test:watch
npm run test:coverage
```

`tests/lib` — чиста логіка (формати, дати, CSV, помилки, арифметика звітів),
`tests/api` — доводить, що ціна не потрапляє в запити учня й класного керівника,
`tests/ui` — компонентні тести ключових екранів, зокрема замовлення за клас. Тести проходять у CI перед збіркою.
...
- [x] **Фаза 5. Дашборди** — порції на день для їдальні (з друком),
      дашборд школи з вартістю в розрізі «пільгові / звичайні», дашборд
      класу без сум, обидва відсотки охоплення, експорт усюди
- [x] **Фаза 6. Адміністрування** — пошук учня по школі, замовлення за
      учня й скасування, переведення між класами, розміри класів і
      керівники, налаштування, переведення року з призначенням керівників,
      видалення даних і журнал видалень
- [x] **Фаза 7. Звіти** — відшкодування, табель харчування, охоплення,
      пільговики, пізні замовлення; друк із шапкою для підпису та CSV
