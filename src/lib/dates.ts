...
/**
 * Дата без часу у форматі бази.
 *
 * ⚠ Саме локальна, а не київська. Уся арифметика нижче працює з parseISO,
 * тобто в локальному часі; якщо форматувати результат у Europe/Kyiv, дата
 * з'їжджає на добу скрізь, де пояс відстає від київського. Наприклад,
 * endOfMonth дає 30 вересня 23:59:59.999 — у Києві це вже 1 жовтня, і звіт
 * за вересень захоплював би перше жовтня.
 *
 * toIsoDate() без аргументів лишається київським: «яке сьогодні число в школі»
 * — питання саме про київський час.
 */
...
export function shiftIso(isoDate: string, days: number): string {
  return isoOf(addDays(parseISO(isoDate), days))
}
...
export function monthBounds(isoDate: string): [string, string] {
  ...
  return [isoOf(startOfMonth(day)), isoOf(endOfMonth(day))]
}
...
export function nextMonthBounds(from: string): [string, string] {
  ...
  return [isoOf(next), isoOf(endOfMonth(next))]
}
